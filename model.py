...
def assign_break_no_intersection(
    ...
    unable = unable.union(set(list(range(candidates[0]-4,candidates[0]+5))))
    current_min_index = 0
    i = 1
    # print('------------------------------')
    # print(f'start_day = {start_day}')
    # print(f'start_lunch = {start_lunch}')
    # print(f'ordered_dict = {ordered_dict}')
    # print('------------------------------')
    while unable & switch != switch:
        if i >= len(candidates):
            current_min_index += 1
            ...
            if (aux >= 5) and (aux <= 9):
                breaks.append(candidates[i])
                unable = unable.union(set(list(range(candidates[i]-4,candidates[i]+5))))
                current_min_index = i
                i += 1
            else:
                i += 1
...
def assign_breaks(start_day_schedule:list, lunch_schedule:list, demand:np.array, schedule:np.array):
    for i in range(len(start_day_schedule)):
        ...
        beginning_day = set(list(range(4,9))) - unable
        # Find the candidate stripes to be active pause before the lunch and exclude 
        # those that cannot change
        if aux - 9 >= 0:
            before_lunch = set(list(range(aux-9,aux-4))) - unable
        else:
            before_lunch = set(list(range(0,aux-4))) - unable
        # Find the candidate stripes to be active pause after the lunch and exclude 
        # those that cannot change
        if aux + 15 <= 38:
            after_lunch = set(list(range(aux+10,aux+15))) - unable
        else:
            after_lunch = set(list(range(aux+10,38))) - unable
        # Find the candidate stripes to be active pause at the end of the day and 
        # exclude those that cannot change
        ending_day = set(list(range(29,34))) - unable
