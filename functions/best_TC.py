...
def search_best_schedule_per_day(demand_per_day, n_workers_TC):
    ...
    for i in range(len(demand_per_day)):
        solution = TC_schedule(n_workers_TC,demand_per_day[i],5_000)
...
def final_solution_per_day_TC(demand_per_day, n_workers_TC):
    ...
    for demand in demand_per_day:
        ...
        solution = assign_breaks(best_solution['start_day_schedule'], best_solution['lunch_schedule'], demand_updated, best_solution['schedule'])
        total_shortfalls += calculate_shortfall(solution,list(demand['demanda']))
        solution_per_day.append(solution)
    return solution_per_day, total_shortfall
